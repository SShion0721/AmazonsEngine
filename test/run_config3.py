"""Config 3: Bounded(10,6) + Null Move + No Categories"""
...
def play_game(gn, ms, uci_white):
    ...
    for k, v in [("Search Mode","Match"),("Use MCTS Root","false"),
                  ...
                  ("Threads","1"),("Hash","512"),
                  ("Use Bounded MoveGen","true"),("Bounded Dest Cap","10"),
                  ("Bounded Arrow Cap","6"),
...
def main():
    ...
    logfile = os.path.join(current_dir, f"config3_log_{ts}.txt")
    resfile = os.path.join(current_dir, f"config3_results_{ts}.json")
    print("Config 3: Bounded(10,6) + Null Move + No Categories")
    t0 = time.time()
    results = [None]*GAMES
    with open(logfile,"w") as f:
        f.write(f"Config 3 | {MS}ms/move | {GAMES} games\n")
    def run_game(gn):
        uci_white = (gn % 2 == 0)
    ...
    print(f"\nConfig 3: {w1}W {dr}D {w2}L | Elo {elo:+.1f} +/- {se:.1f} | Score {sc:.1f}%")
    with open(resfile,"w") as f:
        json.dump({"config":"Bounded(10,6)+NullMove+NoCategories","w1":w1,"w2":w2,"draws":dr,"elo":elo,"se":se,"score":sc}, f, indent=2)
