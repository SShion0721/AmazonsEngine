"""
AmazonsEngine (UCI) vs Amazon Champion (MCTS)
12 parallel workers, fresh engines per game, 0.1s/move, 100 games
"""
...
def uci_go(proc, uci_moves, ms):
    """Send go command and get bestmove using binary pipes."""
    mv = " ".join(uci_moves)
...
def play_game(gn, ms, uci_white):
    """Play one game with fresh engine processes."""
    g, [wq, bq] = init_board()
    uci_side = W if uci_white else B
    uci_moves = []
    n = 0

    # Start fresh UCI engine
    ...
    while time.time()-s < 10:
        ...
        if not l: return (gn, -1, -2)  # engine died
        if "uciok" in l.decode(errors="replace"): break
    else:
        return (gn, -1, -2)

    for k, v in [("Search Mode","Match"),("Use MCTS Root","true"),
                  ("Use Strong Classical","true"),("Eval Tier","Strong"),
                  ("Threads","1"),("Hash","512"),
                  ("Use Bounded MoveGen","false"),
                  ("Use Soft Tail Search","true"),
                  ("Use Null Move","false"),
                  ("Use LMP","false")]:
...
def main():
    ...
    logfile = os.path.join(current_dir, f"match_log_{ts}.txt")
    resfile = os.path.join(current_dir, f"match_results_{ts}.json")

    print("="*60)
    print(f"AmazonsEngine vs Amazon Champion MCTS")
    print(f"Both: 1 thread, {MS}ms/move")
    print(f"Games: {GAMES}, Parallel: {PAR}")
    print(f"Fresh engine per game, timeout: {GAME_TIMEOUT}s")
    print("="*60)
    ...
    with open(logfile,"w") as f:
        f.write(f"AmazonsEngine vs Amazon Champion MCTS | {MS}ms/move | {GAMES} games\n")
    ...
    with ThreadPoolExecutor(max_workers=PAR) as pool:
        ...
        for fut in as_completed(futures):
            ...
            if nm == -2:
                ...
                tag = "E1+" if res==1 else "E2+" if res==-1 else "Draw"
                nm_str = f"{nm:3d}mv"

            sw = "W" if gn%2==0 else "B"
            elapsed = time.time()-t0
            done = [r for r in results if r is not None]
            w1 = sum(1 for r,_ in done if r==1 and _>=0)
            w2 = sum(1 for r,_ in done if r==-1 and _>=0)
            dr = sum(1 for r,_ in done if r==0 and _>=0)
            tmo = sum(1 for r,_ in done if _<0)
            status = f"E1 {w1} - {dr} - {w2} E2"
            if tmo: status += f" ({tmo} err/timeout)"
            print(f"[{done_count:3d}/{GAMES}] G{gn+1:3d} {tag} {nm_str} E1={sw}  |  {status}  |  {elapsed:.0f}s")

            with open(logfile,"a") as f:
                f.write(f"G{gn+1}: {tag} {nm_str} E1={sw}\n")

    elapsed = time.time()-t0
    valid = [(r,m) for r,m in results if m >= 0]
    timeouts = sum(1 for _,m in results if m < 0)
    gr = [r for r,_ in valid]
    w1 = sum(1 for r in gr if r==1)
    w2 = sum(1 for r in gr if r==-1)
    dr = sum(1 for r in gr if r==0)
    tm = sum(m for _,m in valid)
    elo, se = calc_elo(results)
    sc = (w1+dr*0.5)/len(gr)*100 if gr else 0

    print("\n"+"="*60)
    print(f"AmazonsEngine  {w1} wins")
    print(f"Amazon MCTS    {w2} wins")
    print(f"Draws          {dr}")
    if timeouts: print(f"Errors/Timeout {timeouts}")
    print(f"Avg moves      {tm/len(gr):.1f}" if gr else "No valid games")
    print(f"Time           {elapsed:.0f}s ({elapsed/GAMES:.1f}s/game)")
    print(f"Elo            {elo:+.1f} +/- {se:.1f}")
    print(f"Score          {sc:.1f}%")
    print("="*60)

    with open(resfile,"w") as f:
        json.dump({
            "ts":ts, "e1":"AmazonsEngine(Match,MCTS,Strong,1T,256MB)",
            "e2":"Amazon Champion MCTS(UCT,1T)", "ms":MS, "games":GAMES,
            "valid":len(gr), "timeouts":timeouts,
            "results":[{"g":i+1,"r":"e1" if r==1 else "e2" if r==-1 else "draw",
                         "w":i%2==0,"m":m} for i,(r,m) in enumerate(results)],
            "w1":w1,"w2":w2,"draws":dr,"elo":elo,"se":se,"score":sc
        }, f, indent=2)
    print(f"Results: {resfile}")
    print(f"Log: {logfile}")
