"""
Ablation test: baseline (no NMP, no LMP, no hard truncation) vs +each feature
Usage: python run_ablation.py <config_name> [extra_uci_options...]
Example: python run_ablation.py baseline
         python run_ablation.py +nmp "Use Null Move=true"
         python run_ablation.py +lmp "Use LMP=true"
         python run_ablation.py +softtail "Use Soft Tail Search=true"
"""
...
# Parse config name and extra UCI options from argv
CONFIG_NAME = sys.argv[1] if len(sys.argv) > 1 else "baseline"
EXTRA_UCI = {}
for arg in sys.argv[2:]:
    if "=" in arg:
        k, v = arg.split("=", 1)
        EXTRA_UCI[k.strip()] = v.strip()
...
def play_game(gn, ms, uci_white):
    ...
    # Base options: disable NMP, LMP, hard truncation
    base_opts = [
        ("Search Mode","Match"), ("Use MCTS Root","false"),
        ("Use Strong Classical","true"), ("Eval Tier","Fast"),
        ("Threads","1"), ("Hash","512"),
        ("Use Bounded MoveGen","true"), ("Bounded Dest Cap","8"),
        ("Bounded Arrow Cap","5"),
        ("Use Null Move","false"),
        ("Use LMP","false"),
        ("Use Soft Tail Search","false"),
    ]
    # Apply extra UCI options from command line
    for k, v in EXTRA_UCI.items():
        # Override base option if it exists, otherwise append
        base_opts = [(kk, vv) if kk != k else (k, v) for kk, vv in base_opts]
        if k not in dict(base_opts):
            base_opts.append((k, v))

    for k, v in base_opts:
...
def main():
    ...
    logfile = os.path.join(current_dir, f"abl_{CONFIG_NAME}_{ts}.txt")
    resfile = os.path.join(current_dir, f"abl_{CONFIG_NAME}_{ts}.json")

    extra_str = ", ".join(f"{k}={v}" for k,v in EXTRA_UCI.items()) or "none"
    print("="*60)
    print(f"Ablation: {CONFIG_NAME} | Extra: {extra_str}")
    print(f"Both: 1 thread, {MS}ms/move | Games: {GAMES} | Parallel: {PAR}")
    print("="*60)

    t0 = time.time()
    results = [None]*GAMES
    with open(logfile,"w") as f:
        f.write(f"Ablation {CONFIG_NAME} | {extra_str} | {MS}ms | {GAMES}g\n")
    ...
    with ThreadPoolExecutor(max_workers=PAR) as pool:
        ...
        for fut in as_completed(futures):
            ...
            else: tag = "W" if res==1 else "L" if res==-1 else "D"
            ...
            print(f"[{done_count:2d}/{GAMES}] G{gn+1:2d} {tag}  {w1}W-{dr}D-{w2}L  | {time.time()-t0:.0f}s")
            with open(logfile,"a") as f:
                f.write(f"G{gn+1}: {tag} E1={'W' if gn%2==0 else 'B'}\n")
    ...
    print(f"\n{'='*60}")
    print(f"{CONFIG_NAME}: {w1}W {dr}D {w2}L | Elo {elo:+.1f} +/- {se:.1f} | Score {sc:.1f}%")
    print(f"Time: {elapsed:.0f}s ({elapsed/GAMES:.1f}s/game)")
    print(f"{'='*60}")

    with open(resfile,"w") as f:
        json.dump({
            "config": CONFIG_NAME, "extra": EXTRA_UCI,
