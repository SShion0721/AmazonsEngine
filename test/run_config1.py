"""Config 1: Bounded(8,5) + Null Move + No Categories"""
...
def main():
    ...
    logfile = os.path.join(current_dir, f"config1_log_{ts}.txt")
    resfile = os.path.join(current_dir, f"config1_results_{ts}.json")
    print("Config 1: Bounded(8,5) + Null Move + No Categories")
    t0 = time.time()
    results = [None]*GAMES
    with open(logfile,"w") as f:
        f.write(f"Config 1 | {MS}ms/move | {GAMES} games\n")
    def run_game(gn):
        uci_white = (gn % 2 == 0)
    ...
    print(f"\nConfig 1: {w1}W {dr}D {w2}L | Elo {elo:+.1f} +/- {se:.1f} | Score {sc:.1f}%")
    with open(resfile,"w") as f:
        json.dump({"config":"Bounded(8,5)+NullMove+NoCategories","w1":w1,"w2":w2,"draws":dr,"elo":elo,"se":se,"score":sc}, f, indent=2)
